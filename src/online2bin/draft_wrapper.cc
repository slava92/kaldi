...
std::string finish_pipeline(OnlineNnet2FeaturePipeline &feature_pipeline,
...
void accept_wave_part(Vector<BaseFloat> &wave_part,
...
std::string get_temporary_transcript(SingleUtteranceNnet3Decoder &decoder,
                                     fst::SymbolTable *word_syms,
                                     bool produce_time,
                                     int32 &frame_offset,
...
std::string get_final_message(SingleUtteranceNnet3Decoder &decoder,
                              int32 &frame_offset,
                              fst::SymbolTable *word_syms,
                              bool produce_time,
                              BaseFloat frame_shift,
...
Vector<BaseFloat> bytes_to_vector(char* data[], int32 length) {
  Vector<BaseFloat> buf;

  buf.Resize(static_cast<MatrixIndexT>(length));

  for (int i = 0; i < length; i++) {
    buf(i) = static_cast<BaseFloat>(data[i]);
  }

  return buf;
}

std::string mockup_for_python(char* data[], length) {
  Vector<BaseFloat> wave_part = bytes_to_vector(data, length);

  ////////////////////////////////////////////////////////////
  // mockup of initialisation section - TBI
  ...
  std::vector<std::pair<int32, BaseFloat>> delta_weights;
  // end of mockup of initialisation section - TBI
  ////////////////////////////////////////////////////////////

  accept_wave_part(wave_part,
                   feature_pipeline,
                   samp_freq,
                   samp_count,
                   chunk_len,
  ...
  std::string msg = get_final_message(decoder,
                                      frame_offset,
                                      word_syms,
                                      produce_time,
                                      frame_shift,
      ...
      while (!eos) {
        ...
        while (true) {
          ...
          if (eos) {
            ...
            std::string msg = finish_pipeline(feature_pipeline,
                                              silence_weighting,
                                              decoder,
                                              frame_offset,
                                              decodable_opts,
          ...
          if (samp_count > check_count) {
            if (decoder.NumFramesDecoded() > 0) {
              ...
              std::string msg = get_temporary_transcript(decoder,
                                                         word_syms,
                                                         produce_time,
                                                         frame_offset,
                                                         frame_shift,
